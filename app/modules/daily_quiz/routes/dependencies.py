...
daily_quiz_submit_rate_limiter = RateLimiter(max_requests=100, window_seconds=3600)
...
def get_correction_service(db: Session = Depends(get_db)):
    from app.modules.daily_quiz.services.daily_quiz_correction import (
        DailyQuizCorrectionService,
    )
