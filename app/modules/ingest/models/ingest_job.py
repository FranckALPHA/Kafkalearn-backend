...
class IngestJob(Base, TimestampMixin):
    ...
    __table_args__ = (
        CheckConstraint("job_type IN ('single_file','folder_scan','csv_batch','worker_embed')", name="ck_ingest_job_type"),
        CheckConstraint("status IN ('pending','running','complete','partial','failed')", name="ck_ingest_job_status"),
