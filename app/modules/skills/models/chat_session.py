...
from modules.users.models.mixins import TimestampMixin
...
class ChatSession(Base, TimestampMixin):
    ...
    skill_predominant = Column(String(20), nullable=True)  # skill le plus utilisé
    matiere = Column(String(100), nullable=True, index=True)  # matière principale
    
    # ─── Compteurs dénormalisés ──────────────────────────────────
    nb_messages = Column(Integer, default=0, CheckConstraint("nb_messages >= 0"))
    nb_generations_reussies = Column(Integer, default=0, CheckConstraint(">= 0"))
    nb_generations_echouees = Column(Integer, default=0, CheckConstraint(">= 0"))
    
    # ─── Satisfaction utilisateur ────────────────────────────────
    note_utilisateur = Column(SMALLINT, CheckConstraint("note_utilisateur BETWEEN 1 AND 5"), nullable=True)
    ...
    # ─── Index composites ────────────────────────────────────────
    __table_args__ = (
        Index('idx_user_updated', 'user_id', 'updated_at'),  # liste triée
        Index('idx_user_pinned', 'user_id', 'is_pinned', 'updated_at'),  # épinglées d'abord
    )
    ...
    def serialize_list_item(self) -> dict:
        """Sérialisation légère pour la liste des sessions."""
        return {
            "id": str(self.id),
            "titre": self.titre,
